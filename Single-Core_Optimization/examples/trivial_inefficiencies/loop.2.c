  ...
  for ( int r = 0; r < nIter; r++ )
    {
      ...
      for ( int p = 0; p < Np; p++ )
	for ( int i = 0; i < Ng; i++ ) {
	  double dx2 = (x[p] - (double)i*Ng_inv + half_size)*(x[p] - (double)i*Ng_inv + half_size);
	  for ( int j = 0; j < Ng; j++ ) {
	    double dy2 = (y[p] - (double)j*Ng_inv + half_size)*(y[p] - (double)j*Ng_inv + half_size);
	    for ( int k = 0; k < Ng; k++ )
	      {
		double dz2 = (z[p] - (double)k*Ng_inv + half_size)*(z[p] - (double)k*Ng_inv + half_size);
